    ...
    def create_group(self):
        ...
        if group_name:
            if group_name not in self.groups:
                ...
                QMessageBox.information(self.main_window, "Успех", f"Группа '{group_name}' создана")
            else:
                QMessageBox.warning(self.main_window, "Ошибка", "Группа с таким именем уже существует")
        else:
            QMessageBox.warning(self.main_window, "Ошибка", "Введите название группы")

    def remove_group(self):
        current_item = self.groups_list.currentItem()
        if current_item:
            group_name = current_item.text()
            reply = QMessageBox.question(self.main_window, "Удаление группы",
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                QMessageBox.information(self.main_window, "Успех", f"Группа '{group_name}' удалена")
        else:
            QMessageBox.warning(self.main_window, "Ошибка", "Выберите группу для удаления")
